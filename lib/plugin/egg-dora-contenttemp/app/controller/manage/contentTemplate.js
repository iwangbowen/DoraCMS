...
let ContentTemplateController = {
    ...
    _setTempData(ctx, app, targetTemp) {
        ...
        //读取ejs模板
        let newPubPath = siteFunc.setTempParentId(ctx.helper.scanFolder(app.config.temp_view_folder, targetTemp + "/public"), 'public');
        // let newUserPath = siteFunc.setTempParentId(ctx.helper.scanFolder(app.config.temp_view_folder, targetTemp + "/users"), 'users');
        // newPubPath = newPubPath.concat(newUserPath);
        // 读取国际化
        // let newI18nPath = siteFunc.setTempParentId(ctx.helper.scanFolder(app.config.temp_locales_folder), 'i18n');
        // newPubPath = newPubPath.concat(newI18nPath);
        let temp_static_folder = app.config.temp_static_folder;
        //读取静态文件
        if (fs.existsSync(temp_static_folder + targetTemp)) {
            let newStylePath = siteFunc.setTempParentId(ctx.helper.scanFolder(temp_static_folder, targetTemp + "/css"), 'styles');
            let newJsPath = siteFunc.setTempParentId(ctx.helper.scanFolder(temp_static_folder, targetTemp + "/js"), 'js');
            newPubPath = newPubPath.concat(newStylePath).concat(newJsPath)
        }
        //读取模板单元
        let filePath = ctx.helper.scanJustFolder(app.config.temp_view_folder + targetTemp);
        let tempUnit = [];
        tempUnit.push({
            id: 'tempUnit',
            parentId: 0,
        ...
        for (let i = 0; i < filePath.length; i++) {
            ...
            if (fileObj.name.split('-')[1] == 'stage') {
                ...
                let unitArr = ctx.helper.scanFolder(app.config.temp_view_folder, targetTemp + '/' + fileObj.name);
        ...
        //读取根目录下的所有文件
        let rootArr = ctx.helper.scanFolder(app.config.temp_view_folder, targetTemp);
        let newRootArr = [];
    ...
    async getFileInfo(ctx, app) {
        ...
        try {
            ...
            if (filePath && (filePath.split('./').length > 1)) {
                ...
                let path = siteFunc.getTempBaseFile(filePath, app.config.temp_view_folder, app.config.temp_static_folder) + filePath;
                if (path) {
