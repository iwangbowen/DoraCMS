...
let ContentController = {
    ...
    async getWordHtmlContent(ctx, app) {
        try {
            ...
            // 上传基础目录
            const uplaodBasePath = process.cwd() + '/app/public/upload';
                ...
                var options = {
                    convertImage: mammoth.images.imgElement(function(image) {
                        ...
                        return image.read("base64").then(function(imageBuffer) {                            
                            ...
                            fs.writeFile(imageFullName, dataBuffer, "binary", function(err){
                                if(err){
                                    console.log("Word 转 Html 图片生成失败。");
                                }				
                            });                        
                            return {
                                src: '/static/upload/images/' + dayStr + '/' + imageName
                            };
                        });                        
                    })
                };

                var result = await mammoth.convertToHtml({path: target}, options);
