...
let ContentController = {
    ...
    checkContentFormData(ctx, fields) {
        ...
        if (!validator.isLength(fields.description, 5, 300)) {
    ...
    async list(ctx, app) {
        ...
        try {
            ...
            let contentList = await ctx.service.content.find(payload, {
                sort: sortObj,
                query: queryObj,
                searchKeys: ['userName', 'title', 'comments', 'description'],
    ...
    async getNearbyContent(ctx, app) {
        ...
        try {
            ...
            let preContent = await ctx.service.content.find({
                ...
                files: 'title _id id data updateDate sImg description'
            });
    ...
    async addContent(ctx, app) {
        ...
        try {
            ...
            const contentFormObj = {
                ...
                isTop: fields.isTop,
                description: xss(fields.description),
    ...
    async uploadFileToContent(ctx, app) {
        try {
            // 获取 steam
            const stream = await ctx.getFileStream();
            // 上传基础目录
            let uploadOptions = app.config.doraUploadFile.uploadFileFormat;
            const uploadBasePath = uploadOptions.upload_path + '/upload';
