...
var siteFunc = {
    ...
    extractfile(ctx, app, DOWNLOAD_DIR, target_path, tempObj, installType) {
        return new Promise((resolve, reject) => {
            ...
            extract.on('finish', async () => {
                ...
                await this._checkDistForder(ctx, app.config.temp_view_folder + tempObj.alias + '/dist', ['images', 'css', 'js']);
                setTimeout(() => {
    ...
    async copyThemeToStaticForder(ctx, app, tempAlias, DOWNLOAD_DIR) {
        ...
        let fromPath = app.config.temp_view_folder + tempAlias + '/dist/*';
        let assetsPath = app.config.temp_view_folder + tempAlias + '/assets/';
        let targetPath = temp_static_folder + tempAlias;
    ...
    async deleteThemeStaticForder(app, tempAlias) {
        ...
        let temp_static_folder = app.config.temp_static_folder;
        var tempPath = app.config.temp_view_folder + tempAlias;
        var tempStaticPath = temp_static_folder + tempAlias;
